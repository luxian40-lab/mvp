...
class EstudianteAdmin(admin.ModelAdmin):
    ...
    list_display = ('cedula_formateada', 'nombre', 'telefono_formateado', 'cliente_nombre', 'grupos_display', 'cursos_inscritos', 'activo', 'fecha_registro')
    ...
    list_per_page = 50
    ordering = ('-fecha_registro',)
    actions = ['enviar_mensaje_masivo', 'enviar_anuncio_grupal', 'invitar_a_grupo_whatsapp', 'exportar_estudiantes_por_curso', 'exportar_plantilla_importacion', 'asignar_a_grupo_accion']
            ...
            for estudiante in queryset:
                try:
                    ...
                    if resultado.get('success'):
                        ...
                        errores += 1
                        errores_detalle.append(f"{estudiante.nombre}")
    ...
    def exportar_plantilla_importacion(self, request):
        ...
        # Solo mostrar mensaje si es una acción de admin (tiene queryset)
...
class CampanaAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('🎯 Plantilla de WhatsApp (Content Template)', {
            'fields': ('plantilla',),
            'description': mark_safe('''<div style="background:#fff3e0;padding:15px;border-radius:8px;border-left:4px solid #ff9800;">
                <strong>⚠️ IMPORTANTE: Content Template Requerido</strong><br><br>
                <strong>Flujo correcto:</strong>
                <ol style="margin:10px 0;">
                    <li>🔵 Ve a <a href="https://console.twilio.com/us1/develop/sms/content-editor" target="_blank" style="color:#2196F3;">Twilio Content Editor</a></li>
                    <li>📝 Crea tu plantilla de WhatsApp y obtén el <strong>Content SID</strong> (ej: HX1234...)</li>
                    <li>⚙️ Configura el SID en el campo <code>twilio_template_sid</code> de la plantilla</li>
...
class CursoAdmin(admin.ModelAdmin):
    ...
    list_display = ('emoji_nombre', 'cliente_nombre', 'total_modulos_display', 'duracion_semanas', 'ver_modulos_link', 'activo', 'orden')
    list_filter = ('activo', 'cliente')
    search_fields = ('nombre', 'descripcion', 'cliente__nombre')
    list_editable = ('activo', 'orden')
    inlines = [ModuloInline]
    actions = ['ver_todos_modulos']
    change_list_template = 'admin/curso_changelist.html'  # 🤖 Template personalizado con botón IA
...
class ArchivoModuloInline(admin.StackedInline):
    ...
    verbose_name_plural = '📁 Archivos Multimedia (Imágenes, Videos, Infografías, PDFs)'
    ...
    fieldsets = (
        ...
        ('Archivo o URL', {
            ...
            'description': 'Puedes subir un archivo O proporcionar una URL externa (YouTube, etc.)'
        }),
        ('Configuración', {
...
class ModuloAdmin(admin.ModelAdmin):
    ...
    actions = ['enviar_archivos_multimedia', 'ver_archivos_multimedia']
    ...
    def enviar_archivos_multimedia(self, request, queryset):
        ...
        from .utils_twilio import enviar_whatsapp_twilio
        import json
...
class PlantillaCertificadoAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('📄 OPCIÓN 1: Subir PDF Personalizado (Recomendado)', {
    ...
    def tipo_plantilla(self, obj):
        """Muestra si usa PDF personalizado o diseño Eki"""
...
class CertificadoAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('📄 Certificado', {
            'fields': ('emitido', 'fecha_emision', 'archivo_pdf', 'vista_previa_certificado')
        }),
        ('📲 Envío', {
...
class ArchivoModuloAdmin(admin.ModelAdmin):
    ...
    list_display = ('modulo', 'tipo_badge', 'titulo', 'tamanio_display', 'disponible_offline', 'descargar_btn', 'activo')
    ...
    fieldsets = (
        ...
        ('📎 Archivo', {
            'fields': ('archivo', 'preview_archivo', 'url_externa', 'tamano_bytes', 'duracion_segundos')
        }),
        ('⚙️ Configuración', {
    ...
    actions = ['marcar_disponible_offline', 'marcar_no_disponible_offline']
