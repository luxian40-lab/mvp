...
class ModuloAltaMicrocontenidosTests(TestCase):
    ...
    def test_unfold_css_separa_botones_submit(self):
        from pathlib import Path

        css = Path('static/admin/css/eki_admin_unfold.css').read_text(encoding='utf-8')
        self.assertIn('body.model-modulo .submit-row', css)
        self.assertIn('gap: 0.65rem 0.9rem', css)
    ...
    def test_alta_minima_curso_numero_titulo_sin_texto_dummy(self):
        """QA P0: crear módulo no exige el truco 'contenido suficiente'."""
        self.client.force_login(self.user)
        r = self.client.post(
            reverse('admin:core_modulo_add'),
            {
        ...
        self.assertIn(r.status_code, (302, 200), msg=r.content[:400] if r.status_code == 200 else '')
        if r.status_code == 200:
            body = r.content.decode('utf-8')
